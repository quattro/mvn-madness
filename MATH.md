...
$$ \mu\in\mathbb R^d, \qquad \Sigma\in\mathrm{Sym}_{++}(d). $$

The parameter manifold is

$$ \mathcal G_d = \mathbb R^d\times \mathrm{Sym}_{++}(d). $$
...
$$ u\in\mathbb R^d, \qquad U\in\mathrm{Sym}(d). $$

The Fisher--Rao metric is

$$ g_{(\mu,\Sigma)}((u,U),(v,V)) = u^\top\Sigma^{-1}v + \frac12 \mathrm{tr} \left( \Sigma^{-1}U\Sigma^{-1}V \right). $$
...
$$ g_{(\mu,\Sigma)}((u,U),(v,V)) = u^\top K v + \frac12\mathrm{tr}(KUKV). $$

This metric is invariant under affine transformations

$$ x\mapsto Ax+b, \qquad A\in GL(d). $$
...
$$ \Gamma_{(\mu,\Sigma)}((u,U),(v,V)) = \left( \Gamma^\mu((u,U),(v,V)), \Gamma^\Sigma((u,U),(v,V)) \right), $$

with

$$ \boxed{ \Gamma^\mu((u,U),(v,V)) = -\frac12 \left( U\Sigma^{-1}v+V\Sigma^{-1}u \right) } $$

and

$$ \boxed{ \Gamma^\Sigma((u,U),(v,V)) = \frac12(uv^\top+vu^\top) - \frac12 \left( U\Sigma^{-1}V+V\Sigma^{-1}U \right). } $$
...
$$ \boxed{ \ddot\mu-\dot\Sigma\,\Sigma^{-1}\dot\mu=0 } $$

and

$$ \boxed{ \ddot\Sigma + \dot\mu\dot\mu^\top - \dot\Sigma\,\Sigma^{-1}\dot\Sigma=0. } $$
...
$$ \boxed{ \ddot\mu+K^{-1}\dot K\,\dot\mu=0 } $$

and

$$ \boxed{ \ddot K-\dot K K^{-1}\dot K - K\dot\mu\dot\mu^\top K =0. } $$
...
$$ g_\mu=\nabla_\mu F, \qquad G_\Sigma=\nabla_\Sigma F. $$

The Riemannian gradient $\mathrm{grad}F=(v_\mu,v_\Sigma)$ is defined by

$$ g_{(\mu,\Sigma)}((v_\mu,v_\Sigma),(a,A)) = g_\mu^\top a+\mathrm{tr}(G_\Sigma A) $$

for all tangent directions $(a,A)$. Hence

$$ \boxed{ v_\mu=\Sigma g_\mu } $$

and

$$ \boxed{ v_\Sigma = 2\Sigma\,\mathrm{sym}(G_\Sigma)\,\Sigma. } $$

The negative Riemannian gradient direction for minimization is

$$ \boxed{ -\mathrm{grad}F = \left( -\Sigma g_\mu, -2\Sigma\,\mathrm{sym}(G_\Sigma)\,\Sigma \right). } $$

The squared Riemannian gradient norm is

$$ \|\mathrm{grad}F\|^2 = v_\mu^\top\Sigma^{-1}v_\mu + \frac12 \mathrm{tr} \left( \Sigma^{-1}v_\Sigma\Sigma^{-1}v_\Sigma \right). $$
...
$$ (\mu(t),\Sigma(t)) = \mathrm{Exp}_{(\mu_0,\Sigma_0)} \left( t(\dot\mu_0,\dot\Sigma_0) \right). $$
...
$$ A=\Sigma_0^{1/2}, \qquad A^{-1}=\Sigma_0^{-1/2}. $$
...
$$ x\in\mathbb R^d, \qquad B\in\mathrm{Sym}(d). $$
...
$$ \boxed{ \mu(t)=\mu_0+A\tilde\mu(t) } $$

and

$$ \boxed{ \Sigma(t)=A\tilde\Sigma(t)A. } $$
...
$$ \tilde\mu(0)=0, \qquad \tilde\Sigma(0)=I, $$

$$ \dot{\tilde\mu}(0)=x, \qquad \dot{\tilde\Sigma}(0)=B. $$

Define

$$ \boxed{ G=(B^2+2xx^\top)^{1/2}. } $$

Let

$$ C(t)=\cosh(tG), \qquad S(t)=\sinh(tG). $$

Use precision-coordinate variables

$$ \Delta(t)=\tilde\Sigma(t)^{-1}, \qquad \delta(t)=\tilde\Sigma(t)^{-1}\tilde\mu(t). $$

Then the normalized IVP solution is

$$ \boxed{ \Delta(t) = I +\frac12(C(t)-I) +\frac12 B(C(t)-I)G^{-2}B -\frac12 S(t)G^{-1}B -\frac12 B S(t)G^{-1}. } $$

and

$$ \boxed{ \delta(t) = -B(C(t)-I)G^{-2}x + S(t)G^{-1}x. } $$

Recover

$$ \boxed{ \tilde\Sigma(t)=\Delta(t)^{-1} } $$

and

$$ \boxed{ \tilde\mu(t)=\tilde\Sigma(t)\delta(t). } $$

In implementation, use linear solves rather than explicit inverses whenever possible:

$$ \tilde\mu(t)=\mathrm{solve}(\Delta(t),\delta(t)). $$
...
$$ G=Q\mathrm{diag}(g_i)Q^\top. $$

Then implement matrix functions spectrally.

For example:

$$ \cosh(tG) = Q\mathrm{diag}(\cosh(tg_i))Q^\top. $$

The products involving $G^{-1}$ and $G^{-2}$ should be evaluated through stable scalar functions:

$$ S(t)G^{-1} = Q \mathrm{diag} \left( \frac{\sinh(tg_i)}{g_i} \right) Q^\top, $$

$$ (C(t)-I)G^{-2} = Q \mathrm{diag} \left( \frac{\cosh(tg_i)-1}{g_i^2} \right) Q^\top. $$

Use the limiting values

$$ \frac{\sinh(tg)}{g}\to t \quad\text{as }g\to0, $$

and

$$ \frac{\cosh(tg)-1}{g^2}\to\frac12t^2 \quad\text{as }g\to0. $$
...
$$ v_\Sigma=-2\Sigma\,\mathrm{sym}(G_\Sigma)\,\Sigma. $$

Then perform the update

$$ \boxed{ (\mu_{k+1},\Sigma_{k+1}) = \mathrm{Exp}_{(\mu_k,\Sigma_k)} \left( \eta_k(v_\mu,v_\Sigma) \right). } $$

Equivalently,

$$ (\mu_{k+1},\Sigma_{k+1}) = \mathrm{Exp}_{(\mu_k,\Sigma_k)} \left( -\eta_k\mathrm{grad}F(\mu_k,\Sigma_k) \right). $$
...
$$ m_p(\xi) = F(p) + \langle\mathrm{grad}F(p),\xi\rangle_p + \frac12 \langle\mathrm{Hess}F(p)[\xi],\xi\rangle_p, $$
...
$$ (\mu_0,\Sigma_0) \quad\text{and}\quad (\mu_1,\Sigma_1), $$
...
$$ L_\Omega= \begin{pmatrix} I&0&0 \cr \mu^\top&1&0 \cr \Omega-\frac12\mu\mu^\top&-\mu&I \end{pmatrix}, $$

and

$$ D_\Theta= \begin{pmatrix} \Theta&0&0 \cr 0&1&0 \cr 0&0&\Sigma \end{pmatrix}. $$

Then

$$ \boxed{ G_\Omega=L_\Omega D_\Theta L_\Omega^\top. } $$
...
$$ \boxed{ E(\Omega)=\frac12\|\log G_\Omega\|_F^2. } $$

The horizontal condition is

$$ \boxed{ (\log G_\Omega)_{13}=0, } $$
...
$$ \Theta(t)=\Sigma(t)^{-1}, \qquad \delta(t)=\Theta(t)\mu(t), $$

and therefore

$$ \Sigma(t)=\Theta(t)^{-1}, \qquad \mu(t)=\Theta(t)^{-1}\delta(t). $$
...
$$ \Omega(\omega)= \begin{pmatrix} 0&\omega \cr -\omega&0 \end{pmatrix}. $$
...
$$ \mu\ne0, \qquad [\Sigma,\mu\mu^\top]\ne0. $$

The smallest nontrivial case is

$$ d=2, \qquad \Sigma=\mathrm{diag}(s_1,s_2), \qquad \mu=(u,v), \qquad s_1\ne s_2, \qquad uv\ne0. $$
...
$$ c(t)=\mathcal N(\mu(t),\Sigma(t)), \qquad t\in[0,1], $$
...
$$ \mathrm{KL}(p_0\|p_1) = \frac12 \left[ \mathrm{tr}(\Sigma_1^{-1}\Sigma_0) + (\mu_1-\mu_0)^\top\Sigma_1^{-1}(\mu_1-\mu_0) -d + \log\frac{\det\Sigma_1}{\det\Sigma_0} \right]. $$

Thus the discretized curve length is

$$ \boxed{ D_n(c) = \sum_{i=0}^{n-1} \sqrt{ J(c(t_i),c(t_{i+1})) }. } $$
...
$$ \Sigma(t)=K(t)^{-1}, \qquad \mu(t)=\Sigma(t)h(t). $$

### Expectation-coordinate curve

The expectation parameters are

$$ m=\mu, \qquad M=\Sigma+\mu\mu^\top. $$
...
$$ d=3, \qquad \kappa(\Sigma)\approx 6.6\times10^5, $$
...
$$ \lambda_{\min}, \qquad \lambda_{\max}. $$

The Hilbert projective distance is

$$ \boxed{ d_H(A,B) = \log\frac{\lambda_{\max}}{\lambda_{\min}}. } $$
...
$$ \mathrm{Exp}_{(\mu,\Sigma)}(t(v_\mu,v_\Sigma)) $$
...
$$ (\mu_0,\Sigma_0), \qquad (\mu_1,\Sigma_1), $$

we do not know a general closed-form expression for

$$ \mathrm{Log}_{(\mu_0,\Sigma_0)}(\mu_1,\Sigma_1). $$
...
$$ \boxed{ \text{Exact Fisher--Rao exponential-map optimization for multivariate normal distributions.} } $$

with an additional experimental module:

$$ \boxed{ \text{Horizontal-lift numerical solver for endpoint geodesics.} } $$
